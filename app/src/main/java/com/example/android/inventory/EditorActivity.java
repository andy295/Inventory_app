...
/**
 * Allows user to create a new tool or edit an existing one.
 */
public class EditorActivity extends AppCompatActivity implements
    ...
    /** Boolean flag that keeps track of whether the tool has been edited (true) or not (false) */
    private boolean mToolHasChanged = false;

    /**
     * OnTouchListener that listens for any user touches on a View, implying that they are modifying
     * the view, and we change the mToolHasChanged boolean to true.
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // in order to figure out if we're creating a new tool or editing an existing one.
        Intent intent = getIntent();
        mCurrentToolUri = intent.getData();

        // If the intent DOES NOT contain a tool content URI, then we know that we are
        // creating a new tool.
        if (mCurrentToolUri == null) {
            // This is a new tool, so change the app bar to say "Add a Tool"
            setTitle(getString(R.string.editor_activity_title_new_tool));

            // Invalidate the options menu, so the "Delete" menu option can be hidden.
            // (It doesn't make sense to delete a tool that hasn't been created yet.)
        ...
        mPriceEditText = (EditText) findViewById(R.id.edit_tool_price);
        mQuantityEditText = (EditText) findViewById(R.id.edit_tool_quantity);
        mSupplierEditText = (EditText) findViewById(R.id.edit_tool_supplier_name);
        mSupplierNumberEditText = (EditText) findViewById(R.id.edit_tool_supplier_number);
    ...
    /**
     * Get user input from editor and save tool into database.
     */
    private void saveTool() {
        ...
        // Check if this is supposed to be a new tool
        // and check if all the fields in the editor are blank
        if (mCurrentToolUri == null &&
                TextUtils.isEmpty(nameString) && TextUtils.isEmpty(priceString) &&
                TextUtils.isEmpty(quantityString) && TextUtils.isEmpty(supplierNameString)
                && TextUtils.isEmpty(supplierNumberString)) {
            // Since no fields were modified, we can return early without creating a new tool.
            // No need to create ContentValues and no need to do any ContentProvider operations.
            return;
        }

        // Create a ContentValues object where column names are the keys,
        // and tool attributes from the editor are the values.
        ContentValues values = new ContentValues();
        values.put(ToolEntry.COLUMN_TOOL_NAME, nameString);
        values.put(ToolEntry.COLUMN_SUPPLIER_NAME, supplierNameString);
        values.put(ToolEntry.COLUMN_SUPPLIER_PHONE_NUMBER, supplierNumberString);
        ...
        values.put(ToolEntry.COLUMN_TOOL_PRICE, price);
        ...
        // Determine if this is a new or existing tool by checking if mCurrentToolUri is null or not
        if (mCurrentToolUri == null) {
            // This is a NEW tool, so insert a new tool into the provider,
            // returning the content URI for the new tool.
            ...
            // Otherwise this is an EXISTING tool, so update the tool with content URI: mCurrentToolUri
            // and pass in the new ContentValues. Pass in null for the selection and selection args
            // because mCurrentToolUri will already identify the correct row in the database that
            // we want to modify.
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        // If this is a new tool, hide the "Delete" menu item.
        if (mCurrentToolUri == null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case android.R.id.home:
                // If the tool hasn't changed, continue with navigating up to parent activity
    ...
    public void onBackPressed() {
        // If the tool hasn't changed, continue with handling back button press
        if (!mToolHasChanged) {
            super.onBackPressed();
            return;
        }
    ...
    /**
     * Prompt the user to confirm that they want to delete this tool.
     */
    private void showDeleteConfirmationDialog() {
        ...
        builder.setPositiveButton(R.string.delete, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                // User clicked the "Delete" button, so delete the tool.
                deleteTool();
            }
        });
    ...
    /**
     * Perform the deletion of the tool in the database.
     */
    private void deleteTool() {
        // Only perform the delete if this is an existing tool.
        if (mCurrentToolUri != null) {
            // Call the ContentResolver to delete the tool at the given content URI.
            // Pass in null for the selection and selection args because the mCurrentToolUri
            // content URI already identifies the tool that we want.
